...
export async function timeout<T>(p: Promise<T>, timeout: number, timeoutValue: T | (() => T)): Promise<T> {
...
export async function timeoutOrError<T>(name: string, p: Promise<T>, time: number): Promise<T> {
